...
from uiautomator2.base import _BaseClient, _WiFiBaseClient
...
class _WiFiDevice(_WiFiBaseClient):
    """WiFi mode device (no ADB required)"""
    ...
    def __call__(self, **kwargs):
        """Create UiObject selector, same as _Device"""
        from uiautomator2._selector import UiObject, Selector
        return UiObject(self, Selector(**kwargs))
    
    def textContains(self, text: str):
        """Convenience method for d(textContains=text)"""
        return self(textContains=text)

    def screenshot(self, filename: Optional[str] = None, format="pillow", display_id: Optional[int] = None):
        """
        Take screenshot of device (WiFi mode)
        ...
        Args:
            ...
            display_id (int): use specific display if device has multiple screen (not supported in WiFi mode, ignored)
        ...
        """
        # WiFi mode uses JSON-RPC takeScreenshot method
        # Note: display_id parameter is ignored in WiFi mode as JSON-RPC doesn't support it
        # takeScreenshot(scale, quality):
        #   - scale: 缩放比例，1 表示 100%（不缩放），0.5 表示 50%，2 表示 200%
        #   - quality: JPEG 压缩质量，范围 0-100，80 表示 80% 质量
        ...
        if base64_data:
            ...
            raise RuntimeError("Failed to take screenshot: takeScreenshot returned None")
    ...
    def dump_hierarchy(self, compressed=False, pretty=False, max_depth: Optional[int] = None) -> str:
        """
        Dump window hierarchy (WiFi mode)
    ...
    def freeze_rotation(self, freezed: bool = True):
        """Freeze/unfreeze rotation (WiFi mode)"""
        self.jsonrpc.freezeRotation(freezed)
    
    def screen_on(self):
        """Turn on screen (WiFi mode)"""
        self.jsonrpc.wakeUp()
    
    def screen_off(self):
        """Turn off screen (WiFi mode)"""
        self.jsonrpc.sleep()
    ...
    def swipe(self, fx, fy, tx, ty, duration: Optional[float] = None, steps: Optional[int] = None):
        ...
        if duration is None:
            duration = self.settings['operation_delay'][1]
        if steps is None:
            steps = SCROLL_STEPS
        with self._operation_delay("swipe"):
            self.jsonrpc.swipe(fx, fy, tx, ty, steps)
    
    def drag(self, sx, sy, ex, ey, duration=0.5):
        """Drag from one point to another point (WiFi mode)"""
    ...
    @property
    def touch(self):
        """Touch controller (WiFi mode)"""
        ACTION_DOWN = 0
        ACTION_MOVE = 2
        ACTION_UP = 1
        
        obj: "_WiFiDevice" = self
    ...
    def pos_rel2abs(self):
        """Convert relative position to absolute (WiFi mode)"""
        size = []
    ...
    def _operation_delay(self, operation_name: str = None):
        """Operation delay context manager (WiFi mode)"""
    ...
    def open_notification(self):
        """Open notification panel (WiFi mode)"""
        return self.jsonrpc.openNotification()
    
    def open_quick_settings(self):
        """Open quick settings (WiFi mode)"""
    ...
    def wait_timeout(self) -> float:
        """wait element timeout (WiFi mode)"""
        return self.settings['wait_timeout']
...
class _Device(_BaseClient):
    ...
    def serial(self) -> str:
        ...
        """
        # WiFi mode: return host:port
        if not hasattr(self, '_dev') or self._dev is None:
            return getattr(self, '_serial', 'wifi_mode')
...
class _AppMixIn(AbstractShell):
    def session(self, package_name: str, attach: bool = False) -> "Session":
        ...
        # Pass self (Device instance) to Session, supports both ADB and WiFi mode
        return Session(self, package_name)
    ...
    def app_current(self):
        ...
        """
        # WiFi mode: use JSON-RPC deviceInfo to get current package
        if not hasattr(self, '_dev') or self._dev is None:
            # WiFi mode: get current package from deviceInfo
            info = self.info
            current_package = info.get('currentPackageName', '')
            if current_package:
                # Try to get activity via dumpsys
                activity = None
                pid = self._pidof_app(current_package)
                
                try:
                    # Get full dumpsys output (don't use grep, as it may not be available)
                    output = self.shell("dumpsys window windows").output
                    import re
                    
                    # Try to find mCurrentFocus line
                    # Example: mCurrentFocus=Window{... com.example.app/.MainActivity}
                    # Or: mCurrentFocus=Window{... com.example.app/com.example.app.MainActivity}
                    focus_match = re.search(r'mCurrentFocus.*?(\S+)/(\S+)', output)
                    if focus_match:
                        pkg, act = focus_match.groups()
                        if pkg == current_package:
                            activity = act
                    
                    # If not found, try mFocusedApp
                    if not activity:
                        focused_match = re.search(r'mFocusedApp.*?(\S+)/(\S+)', output)
                        if focused_match:
                            pkg, act = focused_match.groups()
                            if pkg == current_package:
                                activity = act
                    
                    # Alternative: use dumpsys activity to get current activity (don't use grep)
                    if not activity:
                        try:
                            # Get full dumpsys activity output and parse in Python
                            act_output = self.shell("dumpsys activity activities").output
                            # Look for mResumedActivity in the output
                            act_match = re.search(r'mResumedActivity.*?(\S+)/(\S+)', act_output)
                            if act_match:
                                pkg, act = act_match.groups()
                                if pkg == current_package:
                                    activity = act
                            
                            # Also try mLastResumedActivity
                            if not activity:
                                act_match = re.search(r'mLastResumedActivity.*?(\S+)/(\S+)', act_output)
                                if act_match:
                                    pkg, act = act_match.groups()
                                    if pkg == current_package:
                                        activity = act
                    ...
                    # If dumpsys fails, return package and pid only
                    return {
                        'package': current_package,
                        'activity': '',
                        'pid': pid
    ...
    def app_install(self, data: str):
        ...
        """
        if not hasattr(self, '_dev') or self._dev is None:
            raise DeviceError("app_install is not supported in WiFi mode. Use ADB mode or manually install the app.")
    ...
    def app_start(self, package_name: str, activity: Optional[str] = None, wait: bool = False, stop: bool = False, use_monkey: bool = False):
        ...
        Args:
            ...
            stop (bool): Stop app before starting the activity
        ...
        if stop:
            ...
            # Wait a bit for the app to fully stop
            time.sleep(0.2)
        ...
        # Add -S flag to force stop if stop=True (as backup)
        if stop:
            args.insert(2, '-S')
        self.shell(args)
    ...
    def app_stop(self, package_name: str):
        ...
        # WiFi mode: use superShell with am force-stop
        if not hasattr(self, '_dev') or self._dev is None:
            # Directly use superShell with am force-stop command
    ...
    def app_info(self, package_name: str) -> Dict[str, Any]:
        ...
        Raises:
            AppNotFoundError
        """
        # WiFi mode: use shell commands to get app info
        if not hasattr(self, '_dev') or self._dev is None:
            # Use dumpsys package to get app info
            try:
                ...
                pm_output = self.shell(['pm', 'list', 'packages']).output
                ...
                label = None
                try:
                    label_match = re.search(r'applicationLabel=([^\s]+)', output)
                    if label_match:
                ...
                return {
                    ...
                    "mainActivity": None,  # Hard to get without parsing APK
                    "size": None,  # Not available via shell
                }
            except AppNotFoundError:
                raise
            except Exception as e:
                raise AppNotFoundError(f"Failed to get app info: {e}", package_name)
...
class _PluginMixIn:
    ...
    def screenrecord(self):
        try:
            ...
            missing_modules = []
            # 检查所有可能的依赖
            if 'imageio' in error_msg:
                missing_modules.append('imageio')
            ...
            if missing_modules:
                ...
                raise ImportError(f"screenrecord requires '{modules_str}' module(s). Install with: {install_cmd}")
            # 如果无法识别具体模块，返回原始错误
            raise ImportError(f"screenrecord import failed: {e}")
...
class WiFiDevice(_WiFiDevice, _AppMixIn, _PluginMixIn, InputMethodMixIn, _DeprecatedMixIn):
    """WiFi mode Device object (no ADB required)"""
    pass
...
class Session(Device):
    ...
    """
    def __init__(self, dev: Union[adbutils.AdbDevice, Device, WiFiDevice, str], package_name: str):
        # Support both ADB serial/AdbDevice and Device instance
        if isinstance(dev, (Device, WiFiDevice)):
            # Already a Device instance, copy its state
            # We can't call super().__init__ with Device, so we copy attributes
            for key, value in dev.__dict__.items():
                if not key.startswith('__'):
                    setattr(self, key, value)
        else:
            # ADB mode: create Device from serial/AdbDevice
            super().__init__(dev)
...
def connect_wifi(address: str, port: int = 9008) -> "WiFiDevice":
    """
    Connect to device via WiFi (IP:PORT), no ADB required
    
    Args:
        address: Device IP address or "IP:PORT" format
        port: Port number (default: 9008), ignored if address contains port
    ...
    Example:
        connect_wifi("192.168.1.100")
        connect_wifi("192.168.1.100:9008")
        connect_wifi("192.168.1.100", 9008)
    
    Note:
        - Requires u2.jar to be running on the device
        - Device must be on the same network as PC
        - No USB debugging or developer mode required
    """
    ...
    if ':' in address:
        ...
        if len(parts) == 2:
            ...
            try:
                ...
                raise ConnectError(f"Invalid port in address: {address}")
    
    return WiFiDevice(address, port)
