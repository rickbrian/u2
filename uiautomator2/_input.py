...
class InputMethodMixIn(AbstractShell):
    ...
    def _setup_ime(self):
        ...
        # WiFi mode: IME installation not supported
        if not hasattr(self, 'adb_device') or self.adb_device is None:
            raise DeviceError("IME installation is not supported in WiFi mode. Please install the IME APK manually or use ADB mode.")
